...
/**
 * Scan direct (non-cache) plugin installs: ~/.claude/plugins/<plugin>/skills/<dir>/SKILL.md
 * Skips 'cache' (handled by scanPluginSkills) and non-directories.
 */
function scanDirectPluginSkills(pluginsRoot, pluginFilter) {
  const out = [];
  if (!fs.existsSync(pluginsRoot)) return out;
  for (const name of fs.readdirSync(pluginsRoot)) {
    if (name === 'cache') continue;
    if (pluginFilter && name !== pluginFilter) continue;
    ...
    let skillsStat;
    try { skillsStat = fs.statSync(skillsDir); } catch { continue; }
...
function entryFromDirectPluginPath(skillMdPath) {
  // path: .../plugins/<plugin>/skills/<skillDir>/SKILL.md  (no cache/owner/version)
  const parts = skillMdPath.replace(/\\/g, '/').split('/');
  const skillsIdx = parts.lastIndexOf('skills');
  if (skillsIdx < 0 || parts.length < skillsIdx + 3) return null;
  const skillDir = parts[skillsIdx + 1];
...
function bootstrap(opts) {
  ...
  if (includePlugins) {
    ...
    // Also scan direct (non-cache) plugin installs, e.g.
    // ~/.claude/plugins/agent-skills/skills/... — not under cache/.
    const pluginsRoot = path.dirname(pluginsDir);
    for (const p of scanDirectPluginSkills(pluginsRoot, pluginFilter)) {
      const e = entryFromDirectPluginPath(p);
      if (e) candidates.push(e);
    }
  }
...
module.exports = {
  ...
  toDisplayPath,
  // scan
  scanCustomSkills, scanPluginSkills, scanDirectPluginSkills,
  // registry
